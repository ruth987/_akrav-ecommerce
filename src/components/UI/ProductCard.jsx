...
const ProductCard = (item) => {
  ...
  return (
    <div className='m-5 shadow-sm hover:-translate-y-1 w-1/5 '>
    

<div className="w-full max-w-sm bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
<a href={`/shop/${item.product.id}`}>
    <img className="p-0 w-full h-56 rounded-t-lg" src={item.product.imgUrl} alt="product image" />
    </a>
<div className="px-5 pb-5">
<a href={`/shop/${item.product.id}`}>
        <h5 className="text-xl font-semibold tracking-tight text-gray-900 dark:text-white py-3">{item.product.productName}</h5>
        </a>
    ...
    <div className="flex items-center justify-between">
        <span className="text-3xl font-bold text-gray-900 dark:text-white">${item.product.price}</span>
          <a
            onClick={addToCart}
